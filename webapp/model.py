...
class User(db.Model):
    ...
    position_type = db.Column(db.Integer, db.ForeignKey('position_type.id'), nullable=False)
...
position_event = db.Table('position_event',
    db.Column('position_type', db.Integer, db.ForeignKey('position_type.id')),
...
class Event(db.Model):
    ...
    positions = db.relationship('Position_type', secondary=position_event,
...
class Schedule(db.Model):
    ...
    # Связь с таблицей user. Позволяет вызвать все расписание для пользователя User.shedules
    user = db.relationship('User',
        backref=db.backref('schedules', lazy='dynamic')
    )
    # Связь с таблицей event. Позволяет вызвать все расписания связанные с событием Event.shedules
